...
char **stringSplit(char ***dest_arr, size_t *len_dest_arr, const char *str, const char *delimiters) {
    ...
    *dest_arr = (char **)realloc((*dest_arr), sizeof(char *) * counter); *len_dest_arr = counter;
    return (*dest_arr);
}
...
vector<string> stringSplit(const string &src, const string &delimiters) {
    ...
    size_t start = 0;
    for (size_t end = src.find_first_of(delimiters, start); end != string::npos; start = end + 1, end = src.find_first_of(delimiters, start)) {
        if (end != start) {
